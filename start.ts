...
type ReactionPhase = "idle" | "waiting" | "showing";
type CompressionPhase = "idle" | "showing" | "input";
type GameId =
  | "memory"
  | "nback"
  | "digits"
...
const initDualNBack = () => {
  const gridEl = document.querySelector<HTMLDivElement>("#nback-grid");
  ...
  const stopBtn = document.querySelector<HTMLButtonElement>("#nback-stop");
  const posMatchBtn = document.querySelector<HTMLButtonElement>("#nback-pos-match");
  const letterMatchBtn = document.querySelector<HTMLButtonElement>("#nback-letter-match");
  ...
  const flashLetterFeedback = (className: "flash-hit" | "flash-miss") => {
  ...
  const showStimulus = (position: number, letter: string) => {
    clearCellHighlights();
  ...
  const handleGuess = (type: "pos" | "letter") => {
    if (state !== "running") {
...
const initReactionInhibition = () => {
  const roundsInput = document.querySelector<HTMLInputElement>("#reaction-rounds");
  const stopInput = document.querySelector<HTMLInputElement>("#reaction-stop");
  const windowInput = document.querySelector<HTMLInputElement>("#reaction-window");
  const startBtn = document.querySelector<HTMLButtonElement>("#reaction-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#reaction-reset");
  const targetBtn = document.querySelector<HTMLButtonElement>("#reaction-target");
  const statusEl = document.querySelector<HTMLParagraphElement>("#reaction-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#reaction-score");
  ...
  const setTargetState = (state: keyof typeof stateLabels, labelOverride?: string) => {
...
const initPatternCompletion = () => {
  const lengthInput = document.querySelector<HTMLInputElement>("#pattern-gap-length");
  const newBtn = document.querySelector<HTMLButtonElement>("#pattern-new");
  const resetBtn = document.querySelector<HTMLButtonElement>("#pattern-reset");
  const sequenceEl = document.querySelector<HTMLDivElement>("#pattern-sequence");
  const optionsEl = document.querySelector<HTMLDivElement>("#pattern-options");
  const statusEl = document.querySelector<HTMLParagraphElement>("#pattern-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#pattern-score");
  ...
  const buildOptions = (value: number, minValue: number, maxValue: number) => {
  ...
  const renderSequence = (sequence: number[], reveal = false) => {
  ...
  const revealAnswer = () => {
    const tiles = sequenceEl.querySelectorAll<HTMLDivElement>(".pattern-tile");
  ...
  const handleOption = (btn: HTMLButtonElement, value: number) => {
    if (solved) return;
    solved = true;
    total += 1;
...
const initMentalRotation = () => {
  const leftCanvas = document.querySelector<HTMLCanvasElement>("#rotation-left");
  const rightCanvas = document.querySelector<HTMLCanvasElement>("#rotation-right");
  const statusEl = document.querySelector<HTMLParagraphElement>("#rotation-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#rotation-score");
  const nextBtn = document.querySelector<HTMLButtonElement>("#rotation-next");
  const resetBtn = document.querySelector<HTMLButtonElement>("#rotation-reset");
  const sameBtn = document.querySelector<HTMLButtonElement>("#rotation-same");
  const differentBtn = document.querySelector<HTMLButtonElement>("#rotation-different");
  const mirrorToggle = document.querySelector<HTMLInputElement>("#rotation-mirror");
  ...
  const rotatePoints = (points: number[][], times: number, size = 4) => {
  ...
  const mirrorPoints = (points: number[][], size = 4) =>
    points.map(([x, y]) => [size - 1 - x, y]);
  ...
  const pointsKey = (points: number[][]) =>
    normalizePoints(points)
      .map(([x, y]) => `${x},${y}`)
      .sort()
      .join("|");

  const rotations = (points: number[][]) =>
    [0, 1, 2, 3].map((turns) => rotatePoints(points, turns));

  const mirrorIsDistinct = (points: number[][]) => {
    const mirror = mirrorPoints(points);
    const mirrorKey = pointsKey(mirror);
    return !rotations(points).some((rot) => pointsKey(rot) === mirrorKey);
  };

  const drawShape = (canvas: HTMLCanvasElement, points: number[][], color: string) => {
  ...
  const answer = (guessSame: boolean) => {
    if (solved) return;
    solved = true;
    total += 1;
...
const initDualTaskJuggle = () => {
  const fieldEl = document.querySelector<HTMLDivElement>("#juggle-field");
  const ballEl = document.querySelector<HTMLDivElement>("#juggle-ball");
  const statusEl = document.querySelector<HTMLParagraphElement>("#juggle-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#juggle-score");
  const questionEl = document.querySelector<HTMLParagraphElement>("#juggle-question");
  const speedInput = document.querySelector<HTMLInputElement>("#juggle-speed");
  const intervalInput = document.querySelector<HTMLInputElement>("#juggle-interval");
  const startBtn = document.querySelector<HTMLButtonElement>("#juggle-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#juggle-reset");
  const trueBtn = document.querySelector<HTMLButtonElement>("#juggle-true");
  const falseBtn = document.querySelector<HTMLButtonElement>("#juggle-false");
  ...
  let mathCorrect = 0;
  let mathTotal = 0;
  let mathTimer: number | null = null;
  ...
  const handleMathAnswer = (value: boolean) => {
...
const initTimeEstimation = () => {
  const targetInput = document.querySelector<HTMLInputElement>("#time-target");
  const randomInput = document.querySelector<HTMLInputElement>("#time-random");
  const distractionsInput = document.querySelector<HTMLInputElement>("#time-distractions");
  const toggleBtn = document.querySelector<HTMLButtonElement>("#time-toggle");
  const resetBtn = document.querySelector<HTMLButtonElement>("#time-reset");
  const faceEl = document.querySelector<HTMLDivElement>("#time-face");
  const faceTitleEl = document.querySelector<HTMLDivElement>("#time-face-title");
  const faceSubEl = document.querySelector<HTMLDivElement>("#time-face-sub");
  const noiseEl = document.querySelector<HTMLDivElement>("#time-noise");
  const statusEl = document.querySelector<HTMLParagraphElement>("#time-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#time-score");
  ...
  const updateScore = () => {
    const format = (value: number | null) => (value === null ? "--" : `${(value / 1000).toFixed(2)}s`);
    const score =
  ...
  const setFaceState = (state: "idle" | "running" | "done", title: string, sub: string) => {
...
const initRuleSwitcher = () => {
  const roundsInput = document.querySelector<HTMLInputElement>("#rule-rounds");
  const switchInput = document.querySelector<HTMLInputElement>("#rule-switch");
  const sizeInput = document.querySelector<HTMLInputElement>("#rule-size");
  const startBtn = document.querySelector<HTMLButtonElement>("#rule-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#rule-reset");
  const leftBtn = document.querySelector<HTMLButtonElement>("#rule-left");
  const rightBtn = document.querySelector<HTMLButtonElement>("#rule-right");
  const stimulusEl = document.querySelector<HTMLDivElement>("#rule-stimulus");
  const stimulusLabelEl = document.querySelector<HTMLSpanElement>("#rule-stimulus-label");
  const statusEl = document.querySelector<HTMLParagraphElement>("#rule-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#rule-score");
  const sizeLegend = document.querySelector<HTMLDivElement>(".rule-pill[data-rule=\"size\"]");
  ...
  let switchErrors = 0;
  let awaiting = false;
  let currentStimulus = { color: "red", shape: "circle", size: "large" } as {
    color: (typeof colors)[number];
    shape: (typeof shapes)[number];
    size: (typeof sizes)[number];
  };

  const ruleMap: Record<RuleName, Record<string, Side>> = {
  ...
  const pickRule = (exclude?: RuleName) => {
    const pool: RuleName[] = includeSize ? ["color", "shape", "size"] : ["color", "shape"];
  ...
  const handleResponse = (side: Side) => {
...
const initDelayedRecall = () => {
  const sizeInput = document.querySelector<HTMLInputElement>("#recall-size");
  const delayInput = document.querySelector<HTMLInputElement>("#recall-delay");
  const startBtn = document.querySelector<HTMLButtonElement>("#recall-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#recall-reset");
  const setEl = document.querySelector<HTMLDivElement>("#recall-set");
  const statusEl = document.querySelector<HTMLParagraphElement>("#recall-status");
  const pulseBtn = document.querySelector<HTMLButtonElement>("#recall-pulse");
  const interferenceScoreEl = document.querySelector<HTMLDivElement>("#recall-interference-score");
  const inputsEl = document.querySelector<HTMLDivElement>("#recall-inputs");
  const submitBtn = document.querySelector<HTMLButtonElement>("#recall-submit");
  const scoreEl = document.querySelector<HTMLDivElement>("#recall-score");
  ...
  let size = 5;
  let delaySec = 60;
  let phase: "idle" | "showing" | "delay" | "recall" = "idle";
  let items: string[] = [];
  let revealTimer: number | null = null;
  let countdownTimer: number | null = null;
  let delayTimer: number | null = null;
  let pulseTimer: number | null = null;
  let pulseOffTimer: number | null = null;
  ...
  const setPulseState = (active: boolean) => {
    pulseActive = active;
  ...
  const submit = () => {
    ...
    const inputs = Array.from(inputsEl.querySelectorAll<HTMLInputElement>("input"));
    let roundCorrect = 0;
...
const initBinaryNoise = () => {
  const roundsInput = document.querySelector<HTMLInputElement>("#noise-rounds");
  const durationInput = document.querySelector<HTMLInputElement>("#noise-duration");
  const levelInput = document.querySelector<HTMLInputElement>("#noise-level");
  const startBtn = document.querySelector<HTMLButtonElement>("#noise-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#noise-reset");
  const fieldEl = document.querySelector<HTMLDivElement>("#noise-field");
  const stimulusEl = document.querySelector<HTMLDivElement>("#noise-stimulus");
  const overlayEl = document.querySelector<HTMLDivElement>("#noise-overlay");
  const statusEl = document.querySelector<HTMLParagraphElement>("#noise-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#noise-score");
  const leftBtn = document.querySelector<HTMLButtonElement>("#noise-left");
  const rightBtn = document.querySelector<HTMLButtonElement>("#noise-right");
...
const initMentalCount = () => {
  const speedInput = document.querySelector<HTMLInputElement>("#count-speed");
  const promptInput = document.querySelector<HTMLInputElement>("#count-prompt");
  const plus2Input = document.querySelector<HTMLInputElement>("#count-plus2");
  const startBtn = document.querySelector<HTMLButtonElement>("#count-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#count-reset");
  const streamEl = document.querySelector<HTMLDivElement>("#count-stream");
  const promptArea = document.querySelector<HTMLDivElement>("#count-prompt-area");
  const countInput = document.querySelector<HTMLInputElement>("#count-input");
  const submitBtn = document.querySelector<HTMLButtonElement>("#count-submit");
  const statusEl = document.querySelector<HTMLParagraphElement>("#count-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#count-score");
...
const initSymbolCompression = () => {
  const sizeInput = document.querySelector<HTMLInputElement>("#compress-size");
  const symbolInput = document.querySelector<HTMLInputElement>("#compress-symbols");
  const exposureInput = document.querySelector<HTMLInputElement>("#compress-exposure");
  const startBtn = document.querySelector<HTMLButtonElement>("#compress-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#compress-reset");
  const submitBtn = document.querySelector<HTMLButtonElement>("#compress-submit");
  const gridEl = document.querySelector<HTMLDivElement>("#compress-grid");
  const inputsEl = document.querySelector<HTMLDivElement>("#compress-inputs");
  const statusEl = document.querySelector<HTMLParagraphElement>("#compress-status");
  const scoreEl = document.querySelector<HTMLDivElement>("#compress-score");
  ...
  let symbolCounts: Record<string, number> = {};
...
const initFalsePatternDetector = () => {
  const roundsInput = document.querySelector<HTMLInputElement>("#detector-rounds");
  const lengthInput = document.querySelector<HTMLInputElement>("#detector-length");
  const subtleInput = document.querySelector<HTMLInputElement>("#detector-subtle");
  const startBtn = document.querySelector<HTMLButtonElement>("#detector-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#detector-reset");
  const patternBtn = document.querySelector<HTMLButtonElement>("#detector-pattern");
  const randomBtn = document.querySelector<HTMLButtonElement>("#detector-random");
  const statusEl = document.querySelector<HTMLParagraphElement>("#detector-status");
  const sequenceEl = document.querySelector<HTMLDivElement>("#detector-sequence");
  const scoreEl = document.querySelector<HTMLDivElement>("#detector-score");
  ...
  const isArithmetic = (sequence: number[]) => {
  ...
  const isRepeatingChunk = (sequence: number[], chunkSize: number) => {
  ...
  const isMirror = (sequence: number[]) =>
    sequence.every((value, index) => value === sequence[sequence.length - 1 - index]);

  const isDiffCycle = (sequence: number[], cycleSize: number) => {
  ...
  const isPatternSequence = (sequence: number[]) =>
  ...
  const generatePatternSequence = () => {
    const builders = useSubtle
      ? [buildArithmeticSequence, buildAlternatingSequence, buildMirrorSequence, buildRepeatSequence, buildDiffCycleSequence]
  ...
  const handleChoice = (choiceIsPattern: boolean) => {
    if (!running) return;
    total += 1;
    roundIndex += 1;
...
const initSpeedCategorization = () => {
  const panel = document.querySelector<HTMLElement>("#speed-categorization");
  const roundsInput = document.querySelector<HTMLInputElement>("#speed-rounds");
  const durationInput = document.querySelector<HTMLInputElement>("#speed-duration");
  const categoriesInput = document.querySelector<HTMLInputElement>("#speed-categories");
  const startBtn = document.querySelector<HTMLButtonElement>("#speed-start");
  const resetBtn = document.querySelector<HTMLButtonElement>("#speed-reset");
  const statusEl = document.querySelector<HTMLParagraphElement>("#speed-status");
  const cardEl = document.querySelector<HTMLDivElement>("#speed-card");
  const buttonsEl = document.querySelector<HTMLDivElement>("#speed-buttons");
  const scoreEl = document.querySelector<HTMLDivElement>("#speed-score");
...
const boot = () => {
  ...
  const switchTitleEl = document.querySelector<HTMLElement>("#switch-title");
  ...
  const setActiveGame = (index: number) => {
